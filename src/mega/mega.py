...
class Mega:
    ...
    def upload(self, filename, dest=None, dest_filename=None, byteio=None, gen_size=None, generator=None):
        ...
        input_file = ( open(filename, 'rb') if byteio is None else byteio ) if generator is None else None

        file_size = ( (
            os.path.getsize(filename) if byteio is None else byteio.getbuffer().nbytes
            ) if gen_size is None else gen_size)
        ...
        if byteio is None and generator is None:
            input_file.close()
        return data
